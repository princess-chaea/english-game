...
async function migrate(uid, body) {
  ...
  const data=await adminDb.runTransaction(async tx=>{const target=await tx.get(targetRef);if(target.exists)throw apiError(409,'RECOVERY_TARGET_EXISTS','현재 계정에 이미 학습 기록이 있어 복구할 수 없어요. 처음 화면에서 새로 시작해 주세요.');let legacyRef,legacySnap;for(const id of ids){const candidate=legacyUsers.doc(id),snap=await tx.get(candidate);if(snap.exists){legacyRef=candidate;legacySnap=snap;break;}}if(!legacySnap)throw apiError(404,'LEGACY_NOT_FOUND','기존 게임 기록을 찾지 못했어요.');const old=legacySnap.data();if(String(old.password||'')!==pin)throw apiError(401,'LEGACY_PIN_MISMATCH','기존 PIN 번호가 일치하지 않아요.');sourceUid=text(old.migratedTo,128);if(!sourceUid||sourceUid===uid)throw apiError(409,'RECOVERY_NOT_NEEDED','복구할 이전 계정을 찾지 못했어요. 기존 기록 가져오기를 이용해 주세요.');const sourceRef=accounts.doc(sourceUid),sourceSnap=await tx.get(sourceRef);if(!sourceSnap.exists)throw apiError(404,'RECOVERY_SOURCE_NOT_FOUND','이전한 계정의 학습 기록을 찾지 못했어요.');const next={...sourceSnap.data(),recoveredAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()};tx.create(targetRef,next);tx.delete(sourceRef);tx.update(legacyRef,{migratedTo:uid,recoveredAt:FieldValue.serverTimestamp()});return next;});
