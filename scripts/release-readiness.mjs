...
if (!guildEffectBlock.includes('adminDb.runTransaction') || !guildEffectBlock.includes('guildEffectPointsSpent: spentPoints + cost') || guildEffectBlock.includes('guildCoins: coins - cost') || !guildEffectBlock.includes("collection('effectInvestments')")) throw new Error('Guild effects must spend unallocated contribution points without reducing lifetime ranking points or personal coins.');
...
if (!mainJs.includes('wbSkillCastCount = 0;') || !mainJs.includes('wbSkillCastCount,\n                            wbComboCount,') || !mainJs.includes('wbRichLockedSkillIds: [...wbRichLockedSkillIds]') || !mainJs.includes('attemptWorldBossRichSkillUnlock') || !mainJs.includes('nextTimer: triggered ? 20 : remaining')) throw new Error('Rich curse purification, 20-second seal cycle, unlock quiz, or reconnect persistence is incomplete.');
...
if (!studentApi.includes('const GUILD_EFFECT_BASE_COST = 100') || !studentApi.includes('const GUILD_EFFECT_LEVEL_COST_STEP = 20') || !guildEffectBlock.includes('guildEffectLevelCost(current.level)')) throw new Error('Guild effect contribution cost must scale from 100P by 20P per level.');
